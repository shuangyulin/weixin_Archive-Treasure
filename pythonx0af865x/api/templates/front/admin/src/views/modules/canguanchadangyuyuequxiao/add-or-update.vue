	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="预约类型" prop="yuyueleixing">
					<el-input v-model="ruleForm.yuyueleixing" placeholder="预约类型" clearable  :readonly="ro.yuyueleixing"></el-input>
				</el-form-item>
...
export default {
    ...
    // 多级联动参数
		...
		info(id) {
			this.$http({
				url: `canguanchadangyuyuequxiao/info/${id}`,
				method: "get"
			}).then(({ data }) => {
				if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `canguanchadangyuyuequxiao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.canguanchadangyuyuequxiaoCrossAddOrUpdateFlag = false;
											this.parent.search();
