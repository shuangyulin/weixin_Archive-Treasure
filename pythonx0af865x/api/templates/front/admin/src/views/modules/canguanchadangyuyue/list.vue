<template>
	<div class="main-content" :style='{"padding":"0px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"flex-end"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 10px 20px","borderRadius":"0 0 8px 8px","textAlign":"right","background":"none","display":"block","width":"auto","fontSize":"inherit","order":"1"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="预约类型" prop="yuyueleixing">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">预约类型</label>
						<el-select clearable v-model="searchForm.yuyueleixing" placeholder="请选择预约类型" >
							<el-option v-for="(item,index) in yuyueleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"right","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}'>
					<el-button class="add" v-if="isAuth('canguanchadangyuyue','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('canguanchadangyuyue','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu8" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('canguanchadangyuyue','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"20px","background":"rgba(255,255,255,1)"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('canguanchadangyuyue','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('canguanchadangyuyue','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('canguanchadangyuyue','预约取消')" @click="canguanchadangyuyuequxiaoCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								预约取消
							</el-button>
							<el-button class="edit" v-if=" isAuth('canguanchadangyuyue','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('canguanchadangyuyue','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<canguanchadangyuyuequxiao-cross-add-or-update v-if="canguanchadangyuyuequxiaoCrossAddOrUpdateFlag" :parent="this" ref="canguanchadangyuyuequxiaoCrossaddOrUpdate"></canguanchadangyuyuequxiao-cross-add-or-update>
...
import canguanchadangyuyuequxiaoCrossAddOrUpdate from "../canguanchadangyuyuequxiao/add-or-update";
	export default {
		data() {
			return {
				yuyueleixingOptions: [],
				...
				canguanchadangyuyuequxiaoCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			canguanchadangyuyuequxiaoCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			canguanchadangyuyuequxiaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.canguanchadangyuyuequxiaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','canguanchadangyuyue');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.canguanchadangyuyuequxiaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.canguanchadangyuyuequxiaoCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
        this.sfshOptions = "是,否,待审核".split(',');
          this.yuyueleixingOptions = "参观,查档".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    params['order'] = 'desc';
          if(this.searchForm.yuyueleixing!='' && this.searchForm.yuyueleixing!=undefined){
            params['yuyueleixing'] = this.searchForm.yuyueleixing
          }
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "canguanchadangyuyue/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	canguanchadangyuyuestatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'canguanchadangyuyue/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    async deleteHandler(id ) {
		...
		await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
			...
			await this.$http({
				url: "canguanchadangyuyue/delete",
				method: "post",
				data: ids
			}).then(async ({ data }) => {
				if (data && data.code === 0) {
