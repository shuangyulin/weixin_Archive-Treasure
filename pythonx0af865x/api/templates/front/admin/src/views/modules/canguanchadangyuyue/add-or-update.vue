	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="预约类型" prop="yuyueleixing">
					<el-select :disabled="ro.yuyueleixing" v-model="ruleForm.yuyueleixing" placeholder="请选择预约类型" >
						<el-option
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="预约类型" prop="yuyueleixing">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="身份证号" prop="shenfenzhenghao">
					<el-input v-model="ruleForm.shenfenzhenghao" placeholder="身份证号" clearable  :readonly="ro.shenfenzhenghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="身份证号" prop="shenfenzhenghao">
...
export default {
    ...
    // 多级联动参数
		...
		info(id) {
			this.$http({
				url: `canguanchadangyuyue/info/${id}`,
				method: "get"
			}).then(({ data }) => {
				if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `canguanchadangyuyue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.canguanchadangyuyueCrossAddOrUpdateFlag = false;
											this.parent.search();
